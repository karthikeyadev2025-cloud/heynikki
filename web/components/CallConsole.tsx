...
export default function CallConsole() {
  ...
  // One voice at a time on the page. The hero's wake-word widget keeps its
  // mic open once enabled; with the console also live, a single utterance
  // was answered TWICE — and the wake widget could hear the console's own
  // reply and answer *that*. The console owns the conversation while a call
  // is active; the widget listens for this and stands down.
  useEffect(() => () => {
    try { window.dispatchEvent(new CustomEvent("nikki:console", { detail: { active: false } })); }
    catch { /* noop */ }
  }, []);

  const announceExclusive = useCallback((active: boolean) => {
    try {
      window.dispatchEvent(new CustomEvent("nikki:console", { detail: { active } }));
  ...
  const endCall = useCallback(() => {
    teardown(); setVoice("idle"); setCallState("ended");
    announceExclusive(false);
  }, [teardown, announceExclusive]);
