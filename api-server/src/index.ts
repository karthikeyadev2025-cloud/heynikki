...
// One route, not two. A second handler for this exact path was registered
// ~2600 lines below and returned a presigned URL as JSON, while this one
// streams the decrypted bytes — Express matches the first, so the JSON
// version had never run, and the shapes are incompatible: the player does
// URL.createObjectURL(await r.blob()) and would have choked on JSON. It
// carried apiLimiter, which this one lacked, so call audio was the one
// unthrottled large download in the API; that limiter moves here.
